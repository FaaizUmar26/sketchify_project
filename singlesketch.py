...
from tkinter import filedialog, messagebox # displaying message boxes in a graphical user interface (GUI) application.
from PIL import ImageTk, Image #image is used for opening manipulating  saving various images
#while image tk is used displaying image on gui.
import cv2 # to work with computer vision tasks and manipulate images and videos.
from tkinter import ttk #to access themed widget classes(buttons,labels)
...
def sketchify():
    ...
    if file_path:
        ...
        # for smoothness #intensity and color of image pixel does not change
...
#for pencil Sketch
        #reduce noise from an image #resuce unwanted change in image pixel to enhance image quality
        blur=cv2.medianBlur(gray, 5)
        invert=255-blur
        psketch = cv2.adaptiveThreshold(invert, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 9, 2)
        #Adaptive thresholding is an image processing technique that adjusts the threshold locally for each pixel based on its
        # neighborhood, improving segmentation in varying lighting conditions.
        #adaptive thresh mean perform adaptive thresholding using mean of the neighbor
#for detail Sketch
        #laplacian filter enhances the borders or edges of object
        #it is done to maintain boundries bwtween different regions
        edges = cv2.Laplacian(gray, cv2.CV_8U, ksize=5) #determine the size of the kernal or filter.
        edges = 255 - edges
        _, dsketch = cv2.threshold(edges, 150, 255, cv2.THRESH_BINARY)
#for charcol Sketch
        #  bilateral filter It smoothens the image by averaging pixel values
        # replacing the intensity value of each pixel with the average value of its neighboring pixels.
        ...
        for i, (image, title) in enumerate(zip(images, titles)):
            #enumerate not only shows you the items in the list but also tell the order or position in the list
